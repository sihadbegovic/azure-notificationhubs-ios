...
+ (NSArray *)parseRegistrations:(NSData *)data error:(NSError * __autoreleasing*)error {
    ...
    if (![xmlParser parse]) {
        if (error) {
            NSString *msg = [NSString
                stringWithFormat:@"Fail to parse registration:%@", [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]];
            // NSLog(@"%@",msg);
