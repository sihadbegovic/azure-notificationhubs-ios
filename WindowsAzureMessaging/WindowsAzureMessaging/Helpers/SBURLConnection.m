...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response {
    ...
    if (statusCode != 200 && statusCode != 201) {
        ...
        NSString *msg = [NSString stringWithFormat:@"URLRequest failed for %@ with status code: %@", [self->_request description],
    ...
    if ([[self->_request HTTPMethod] isEqualToString:@"POST"] &&
        [[self->_response URL].path rangeOfString:@"/registrationids" options:NSCaseInsensitiveSearch].location != NSNotFound) {
        self->_completion(self->_response, nil, nil);
        self->_completion = nil;
        return;
    }
